...
def extract_heizenergie_generic(data: dict, tag: str, section_header_name: str) -> str:
    ...
    extracted_data = map(lambda x: {'monat': x.get('monat'),
                                    'jahr': x.get('jahr'),
                                    'table_header': datetime.strptime(f"{x.get('monat')} {x.get('jahr')}",
                                                                      '%m %Y').strftime('%b %y'),
...
def extract_heizenergie_liegenschaft_kwh(html_source: str) -> str:
    ...
    result_html = extract_heizenergie_generic(data, "verbrauchswert_liegenschaft_qm_kwh",
                                              "verbrauchswert_liegenschaft_qm_kwh")
    result_html += extract_heizenergie_generic(data, "verbrauchswert_liegenschaft_geraete_kwh",
...
def extract_kaltwasser_liegenschaft_m3(html_source: str) -> str:
    ...
    result_html = extract_heizenergie_generic(data, "verbrauchswert_liegenschaft_geraete",
                                              "verbrauchswert_liegenschaft_geraete_wasser")
    result_html += extract_heizenergie_generic(data, "verbrauchswert_liegenschaft_qm",
